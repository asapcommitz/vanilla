...
(function () {
	...
	style.textContent = `/* PrismJS 1.30.0
...
.token.treeview-part .entry-name.ext-pptx:before {
	...
	*/
	...
	class ContentElement extends HTMLElement {
		async connectedCallback() {
			...
			try {
				...
				// Dispatch content-loaded for any external listeners
	...
	document.addEventListener("DOMContentLoaded", () => {
		...
		const sidebarElement = document.querySelector("[data-vanillamd-sidebar]");
		if (sidebarElement) {
			...
			const buildTocTree = (headings, container) => {
				...
				let currentLevel = 0;
				let parentStack = [treeRoot];

				headings.forEach((h, index) => {
					if (h.level === 1) return; // Skip title
					while (h.level <= currentLevel && parentStack.length > 1) {
						parentStack.pop();
						currentLevel--;
					}
					const itemWrap = document.createElement('div');
					itemWrap.className = `nav-item-level-${h.level}`;
					const link = document.createElement('a');
					link.href = `#${h.id}`;
					link.textContent = h.text;
					link.className = 'nav-link sub-link';
					const hasChildren = index + 1 < headings.length && headings[index + 1].level > h.level;
					if (hasChildren) {
						const details = document.createElement('details');
						const summary = document.createElement('summary');
						...
						subContent.className = 'nav-children';
						details.appendChild(subContent);
						itemWrap.appendChild(details);
						parentStack[parentStack.length - 1].appendChild(itemWrap);
						parentStack.push(subContent);
			...
			// Initialize wrappers
			sidebarElement.querySelectorAll('a').forEach(wrapSidebarLink);

			// Handle the 'active' page loading
